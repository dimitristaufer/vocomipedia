...
def main() -> int:
    ...
    ap.add_argument("--backup-dir", default=Path("vocomipedia/reports/backups"), type=Path)
    ap.add_argument("--skip-smoke-release", action="store_true")
    args = ap.parse_args()

    root = repo_root_from_tool()
    backup = create_backup(
        paths=[root / "vocomipedia" / "data", root / "vocomipedia" / "catalog"],
    ...
    run([sys.executable, "-m", "py_compile", *[str(p) for p in sorted((root / "vocomipedia" / "tools").glob("*.py"))]], cwd=root)
    run([sys.executable, "-m", "unittest", "vocomipedia.tests.test_pipeline", "-v"], cwd=root)
    ...
    with tempfile.TemporaryDirectory() as td:
        ...
        assets = tmp / "assets"
        assets.mkdir()
        sample = root / "vocomipedia" / "tests" / "fixtures" / "sample_legacy.json"
        ...
        run(
            [
                sys.executable,
                "vocomipedia/tools/import_legacy_pack.py",
                "--pack-code",
                "ja_n5",
                "--input-json",
                str(sample_copy),
        ...
        run([sys.executable, "vocomipedia/tools/validate_corpus.py", "--root", str(pack_dir), "--strict-media"], cwd=root)
        run(
            [
                sys.executable,
                "vocomipedia/tools/release_pack.py",
                "--pack-dir",
                str(pack_dir),
                "--pack-generation-dir",
                ...
                str(root / "vocomi_pack_generation" / "ios_private.pem"),
                "--chunk-mb",
                "1",
            ],
            cwd=root,
