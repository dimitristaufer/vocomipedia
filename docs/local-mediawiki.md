...
```bash
python3 tools/local_mediawiki.py reset
```
...
```text
docker/local/.env
docker/local/LocalSettings.php
```

Both files are ignored by git. The `.env` file contains local admin and bot
credentials.

The local MediaWiki image is built from `docker/mediawiki/Dockerfile`
...
docker compose --env-file docker/local/.env \
  -f docker/compose.local.yml build mediawiki
docker compose --env-file docker/local/.env \
...
docker exec docker-db-1 sh -c 'mariadb-dump -u root -p"$MARIADB_ROOT_PASSWORD" mediawiki' \
  > backups/local-mediawiki-before-change.sql
```

## Day-To-Day Commands

```bash
python3 tools/local_mediawiki.py start
python3 tools/local_mediawiki.py stop
python3 tools/local_mediawiki.py status
python3 tools/local_mediawiki.py refresh-filter
python3 tools/local_mediawiki.py reindex-search
python3 tools/local_mediawiki.py destroy
```
...
```bash
python3 tools/reindex_mediawiki_search.py
```
